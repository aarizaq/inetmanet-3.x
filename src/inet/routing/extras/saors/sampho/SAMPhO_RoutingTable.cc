...
bool comp_function(SAORSBase_RoutingEntry *entry1, SAORSBase_RoutingEntry *entry2) { return (entry1->beaconsRcvd < entry2->beaconsRcvd); }


/*****************************************************************************************
 * The Constructor function of the PS-DYMO routing table class.
 *****************************************************************************************/
SAMPhO_RoutingTable::SAMPhO_RoutingTable(cObject* host, const IPv4Address& myAddr) : SAORSBase_RoutingTable(host, myAddr)
{
	// get our host module
 ...
 *****************************************************************************************/
SAMPhO_RoutingTable::~SAMPhO_RoutingTable() {
 ...
 *****************************************************************************************/
void SAMPhO_RoutingTable::ageAllBeacons() {

    SAORSBase_RoutingEntry *entry=NULL;

	double timeout = dynamic_cast <SAMPhO*> (dymoProcess)->BEACON_TIMEOUT;
 ...
 *****************************************************************************************/
void SAMPhO_RoutingTable::findNeighbors() {
	...
	sort(dtrouteVector.begin(),dtrouteVector.end(),comp_function);
 ...
 *****************************************************************************************/
uint SAMPhO_RoutingTable::getNumOfNeighbors() {
//	uint numOfFriends=floor((dynamic_cast <SAMPhO*> (dymoProcess)->SOCIAL_SENSITIVITY)*dtrouteVector.size()/100);
...
//This code would calculate the social neighbors according to the metric of the most close friend
	...
	if(!dtrouteVector.empty()) {
		...
		double threshold = (*iter)->beaconsRcvd - ((*iter)->beaconsRcvd*(dynamic_cast <SAMPhO*> (dymoProcess)->SOCIAL_SENSITIVITY)/100);
...
/*****************************************************************************************
 * Returns the number of the neighbors to be considered as strangers.
 *****************************************************************************************/
uint SAMPhO_RoutingTable::getNumOfStrangers() {
//	uint numOfStrangers=floor((dynamic_cast <SAMPhO*> (dymoProcess)->ANTISOCIAL_SENSITIVITY)*dtrouteVector.size()/100);

//This code would calculate the social strangers according to the metric of the most close friend
	uint numOfStrangers=0;
	...
	if(!dtrouteVector.empty()) {
		...
		//Get the threshold for considering a node stranger
		double threshold = ((*iter)->beaconsRcvd*(dynamic_cast <SAMPhO*> (dymoProcess)->ANTISOCIAL_SENSITIVITY)/100);

		//Count the met nodes that are considered as strangers
		for(iter=dtrouteVector.begin();iter<dtrouteVector.end();++iter) {
			if((*iter)->beaconsRcvd < threshold)
				numOfStrangers++;
		}
 ...
 *****************************************************************************************/
bool SAMPhO_RoutingTable::isFriend(const SAORSBase_RoutingEntry *entry) {
    if(entry) {
...
/*****************************************************************************************
 * Returns whether a certain routing entry describes a stranger to the current node or not.
 *****************************************************************************************/
bool SAMPhO_RoutingTable::isStranger(const SAORSBase_RoutingEntry *entry) {
    if(entry) {
        //Check is the received entry is member of the neighbors groups
        if( dtrouteVector.size() - findEntryRank(entry->routeAddress.getInt()) > getNumOfStrangers() )
            return false;
	}
 ...
 *****************************************************************************************/
uint SAMPhO_RoutingTable::findEntryRank(uint32_t Address) {
	uint rank=0;
	std::vector<SAORSBase_RoutingEntry *>::iterator iter;
...
*****************************************************************************************/
double SAMPhO_RoutingTable::getBetweenness() {
 ...
 *****************************************************************************************/
void SAMPhO_RoutingTable::updateBeaconCounter(uint32_t Address, VectorOfSAORSBeaconBlocks entryVector) {
    ...
    if(entry) {
        ...
        //Compute the average time that a next meeting should take place
        double avgInterval = (BEACON_COUNTER_WINDOW - entry->beaconsRcvd*dynamic_cast <SAMPhO*> (dymoProcess)->BEACON_TIMEOUT) / (entry->meetings);
        ...
        if( meeting_interval > 2*dynamic_cast <SAMPhO*> (dymoProcess)->BEACON_TIMEOUT*BEACON_THRESHOLD) {
 ...
 * met node.
 *****************************************************************************************/
void SAMPhO_RoutingTable::updateSimilarity(uint32_t Address, VectorOfSAORSBeaconBlocks entryVector) {
 ...
 *****************************************************************************************/
uint SAMPhO_RoutingTable::getMaxBeaconsNumber() {
	return(floor(BEACON_COUNTER_WINDOW/dynamic_cast <SAMPhO*> (dymoProcess)->BEACON_TIMEOUT));
}
 ...
 *****************************************************************************************/
double SAMPhO_RoutingTable::getSimilarityScore(VectorOfSAORSBeaconBlocks beaconEntries) {
...
std::ostream& operator<<(std::ostream& os, const SAMPhO_RoutingTable& o)
{
	os << o.info();
	return os;
}
