 ...
 *****************************************************************************************/
void EPDYMO::handleBeacon(SAORS_BEACON* my_beacon) {
	...
	for(uint i=0;i<dtqueuedDataPackets->getSize();i++) {
        ...
        // Dequeue packets to met carrier. Take into account that the past carrier are handled by the DT Data Queue.
        int numOfPkts=dtqueuedDataPackets->spreadPacketsTo(addr, (dtentry)?dtentry->routePrefix:32, dst_entry->routeAddress, previousPkts);
        flag=true;
 ...
 *****************************************************************************************/
double EPDYMO::findEncounterProb(const SAORSBase_RoutingEntry* routeToNode) {
	return 0;
}
 ...
 *****************************************************************************************/
bool EPDYMO::compareEncounterProb(const SAORSBase_RoutingEntry* dtEntry, const SAORS_RREP* rrep) {
	return false;
}
 ...
 *****************************************************************************************/
bool EPDYMO::sendEncounterProb(SAORS_RREQ* rreq, const SAORSBase_RoutingEntry* dtEntry) {
	return false;
}
