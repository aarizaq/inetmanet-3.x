...
void TraCIMobility::preInitialize(std::string external_id, const Coord& position, std::string road_id, double speed, double angle)
{
	...
	Coord nextPos = calculateAntennaPosition(roadPosition);
	nextPos.z =  par("initialZ");
	lastPosition  = nextPos;

    par("initialX").setDoubleValue(lastPosition.x);
    par("initialY").setDoubleValue(lastPosition.y);
    par("initialZ").setDoubleValue(lastPosition.z);
...
void TraCIMobility::setTargetPosition()
{
	...
	currentPosXVec.record(getCurrentPosition().x);
	currentPosYVec.record(getCurrentPosition().y);
...
{
	Coord pos = lastPosition;
	Coord dummy = Coord::ZERO;
	double dum;

	bool outsideX = (pos.x < 0) || (pos.x >= getConstraintAreaMax().x);
	bool outsideY = (pos.y < 0) || (pos.y >= getConstraintAreaMax().y);
	bool outsideZ = (pos.z < 0) || (pos.z >= getConstraintAreaMax().z);
	if (outsideX || outsideY || outsideZ) {
		error("Tried moving host to (%f, %f) which is outside the playground", pos.x, pos.y);
	}

	handleIfOutside( RAISEERROR, pos, dummy, dum);
}
...
inet::Coord TraCIMobility::calculateAntennaPosition(const Coord& vehiclePos) const {
	Coord corPos;
...
inet::TraCIMobility* TraCIMobilityAccess::get(cModule* host) {
