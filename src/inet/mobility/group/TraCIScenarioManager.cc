        ...
        while (bytesRead < sizeof(uint32_t))
        {
            ...
            else
            {
                ...
                if (sock_errno() == EAGAIN)
                    continue;
                throw cRuntimeError("Connection to TraCI server lost. Check your server's log. Error message: %d: %s", sock_errno(),
                        strerror(sock_errno()));
            }
        }
...
TraCIScenarioManager::TraCIBuffer TraCIScenarioManager::queryTraCI(uint8_t commandId, const TraCIBuffer& buf)
{
    ...
    if (result == RTYPE_NOTIMPLEMENTED)
        throw cRuntimeError("TraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.", commandId,
                description.c_str());
    if (result == RTYPE_ERR)
...
TraCIScenarioManager::TraCIBuffer TraCIScenarioManager::queryTraCIOptional(uint8_t commandId, const TraCIBuffer& buf,
...
void TraCIScenarioManager::connect()
{
    ...
    {
        throw cRuntimeError(
                "Could not connect to TraCI server. Make sure it is running and not behind a firewall. Error message: %d: %s",
...
{
    {
        ...
        {
            throw cRuntimeError("TraCI server \"%s\" reports API version %d. This server is unsupported.", serverVersion.c_str(),
                    apiVersion);
        }
...

    {
        ...
        TraCIBuffer buf = queryTraCI(CMD_GET_SIM_VARIABLE,
                TraCIBuffer() << static_cast<uint8_t>(VAR_NET_BOUNDING_BOX) << std::string("sim0"));
...

    {
        ...
        TraCIBuffer buf = queryTraCI(CMD_SUBSCRIBE_SIM_VARIABLE,
                TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1 << variable2
...

    {
        ...
        TraCIBuffer buf = queryTraCI(CMD_SUBSCRIBE_VEHICLE_VARIABLE,
                TraCIBuffer() << beginTime << endTime << objectId << variableNumber << variable1);
        processSubcriptionResult(buf);
        ASSERT(buf.eof());
    }
...
void TraCIScenarioManager::commandSetSpeed(std::string nodeId, double speed)
{
    ...
    TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << nodeId << variableType << speed);
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::commandNewRoute(std::string nodeId, std::string roadId)
{
    ...
    TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << nodeId << variableType << roadId);
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::commandSetVehicleParking(std::string nodeId)
{
    ...
    TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << nodeId << variableType << value);
    ASSERT(buf.eof());
}
...
void TraCIScenarioManager::commandChangeRoute(std::string nodeId, std::string roadId, double travelTime)
{
    if (travelTime >= 0)
    {
        ...
        TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
                TraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << newTimeT
                        << newTime);
        ASSERT(buf.eof());
    }
    else
    {
        ...
        std::string edgeId = roadId;
        TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
                TraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId);
        ASSERT(buf.eof());
    }
    {
...
double TraCIScenarioManager::commandDistanceRequest(Coord position1, Coord position2, bool returnDrivingDistance)
{
    ...
    // query road network boundaries
    TraCIBuffer buf = queryTraCI(CMD_GET_SIM_VARIABLE,
            TraCIBuffer() << variable << simId << variableType << count << p1 << p2 << dType);
    uint8_t cmdLength_resp;
    buf >> cmdLength_resp;
    uint8_t commandId_resp;
    buf >> commandId_resp;
...
void TraCIScenarioManager::commandStopNode(std::string nodeId, std::string roadId, double pos, uint8_t laneid,
        double radius, double waittime)
{
    ...
    TraCIBuffer buf = queryTraCI(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << nodeId << variableType << count << edgeIdT << edgeId << stopPosT << stopPos
...
{
    TraCIBuffer buf = queryTraCI(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_PROGRAM) << trafficLightId << static_cast<uint8_t>(TYPE_STRING)
                    << program);
    ASSERT(buf.eof());
}
...
{
    TraCIBuffer buf = queryTraCI(CMD_SET_TL_VARIABLE,
            TraCIBuffer() << static_cast<uint8_t>(TL_PHASE_INDEX) << trafficLightId
...
void TraCIScenarioManager::commandAddPolygon(std::string polyId, std::string polyType, const TraCIColor& color,
...
void TraCIScenarioManager::commandAddPoi(std::string poiId, std::string poiType, const TraCIColor& color, int32_t layer,
        Coord pos)
{
    TraCIBuffer p;
...
bool TraCIScenarioManager::commandAddVehicle(std::string vehicleId, std::string vehicleTypeId, std::string routeId,
        simtime_t emitTime_st, double emitPosition, double emitSpeed, int8_t emitLane)
{
    ...
    TraCIBuffer buf = queryTraCIOptional(CMD_SET_VEHICLE_VARIABLE,
            TraCIBuffer() << variableId << vehicleId << variableType << count << (uint8_t) TYPE_STRING << vehicleTypeId
...
void TraCIScenarioManager::addModule(std::string nodeId, std::string type, std::string name, std::string displayString,
        const Coord& position, std::string road_id, double speed, double angle)
{
...
void TraCIScenarioManager::deleteModule(std::string nodeId)
{
    ...
    if (!mod->getSubmodule("notificationBoard"))
        throw cRuntimeError("host has no submodule notificationBoard");
...
std::string TraCIScenarioManager::genericGetString(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
Coord TraCIScenarioManager::genericGetCoord(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
double TraCIScenarioManager::genericGetDouble(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
std::list<std::string> TraCIScenarioManager::genericGetStringList(uint8_t commandId, std::string objectId,
...
std::list<Coord> TraCIScenarioManager::genericGetCoordList(uint8_t commandId, std::string objectId, uint8_t variableId,
        uint8_t responseId)
{
...
{
    return TraCICoord(coord.x + netbounds1.x - margin,
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            throw cRuntimeError("TraCI server reported error subscribing to variable 0x%2x (\"%s\").", variable1_resp,
                    description.c_str());
        }
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isSubscribed)
            {
                ...
                throw cRuntimeError("TraCI server reported error subscribing to vehicle variable 0x%2x (\"%s\").", variable1_resp,
                        errormsg.c_str());
            }
        }
