...
#include "inet/neta/hackedmodules/networklayer/manetrouting/aodv/NA_aodv_uu_omnet.h"
...
bool NA_AODVUU::iswrite = false;
int NA_AODVUU::totalSend=0;
int NA_AODVUU::totalRreqSend=0;
int NA_AODVUU::totalRreqRec=0;
int NA_AODVUU::totalRrepSend=0;
int NA_AODVUU::totalRrepRec=0;
int NA_AODVUU::totalRrepAckSend=0;
int NA_AODVUU::totalRrepAckRec=0;
int NA_AODVUU::totalRerrSend=0;
int NA_AODVUU::totalRerrRec=0;
#endif

void NA_AODVUU::initialize(int stage)
{
     /*
...
/* NETATTACKS: handle menssage from controller. */
...
/* Called for packets whose delivery fails at the link layer */
void NS_CLASS packetFailedMac(ieee80211::Ieee80211DataFrame *dgram)
{
    rt_table_t *rt_next_hop, *rt;
    struct in_addr dest_addr, src_addr, next_hop;
...
void NS_CLASS recvAODVUUPacket(cMessage * msg)
{
    ...
    for (int i = 0; i < getNumWlanInterfaces(); i++)
    {
        ...
        if (!isInMacLayer())
        {
            ...
            if (L3Address(ipv4data->getIPAddress()) == src.s_addr)
            {
                delete   aodv_msg;
                return;
            }
...
void NS_CLASS processLinkBreak(const cObject *details)
{
    ...
    if (llfeedback)
    {
        ...
        else if (dynamic_cast<ieee80211::Ieee80211DataFrame *>(const_cast<cObject*> (details)))
        {
            ieee80211::Ieee80211DataFrame *frame = dynamic_cast<ieee80211::Ieee80211DataFrame *>(const_cast<cObject*>(details));
