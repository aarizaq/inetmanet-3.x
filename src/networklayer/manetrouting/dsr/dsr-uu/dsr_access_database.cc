...
struct dsr_srt *DSRUU::ph_srt_find_map(struct in_addr src, struct in_addr dst, unsigned int timeout)
{
    ...
    if (etxActive)
    {
        ...
        if (!pathCacheMap.getPaths(ManetAddress(IPv4Address(dst.s_addr)), result, resultCost))
            return NULL;
        ...
        for (unsigned int i = 0; i < result.size(); i++)
        {
            ...
            if (!resultCost.empty())
            {
                double cost;
                if (!result[i].empty())
                    cost = getCost(result[i].front().getIPv4());
                else
    ...
    {
        double cost;
        if (!pathCacheMap.getPath(ManetAddress(IPv4Address(dst.s_addr)),route,cost,timeout))
            return NULL;
    }
    ...
    if (oportunity)
    {
        ...
        if (!is_first && !is_last && pos == -1)
        {
            ...
            for (unsigned int i = 0; i < srt->addrs.size(); i++)
            {
                ...
                ManetAddress addr(IPv4Address(srt->addrs[i].s_addr));
            ...
            for (unsigned int i = 0; i < neigh.size(); i++)
            {
                ...
                {
                    ManetAddress addr(IPv4Address(srt->addrs[j].s_addr));
                    val.push_back(addr);
                }
                // reverse path
                Direct.push(val);
...
{
    ManetAddress destinationAddress(IPv4Address(node.s_addr));
    PathCacheRoute route;
    PathCacheCost costVect;
    if (etxActive)
    {
        costVect.push_back(getCost(destinationAddress.getIPv4()));
    }
...
void DSRUU::ph_srt_delete_node_map(struct in_addr src)
{
    ...
    ManetAddress destinationAddress(IPv4Address(src.s_addr));
...
void DSRUU::ph_srt_delete_link_map(struct in_addr src1,struct in_addr src2)
{
    ...
    ManetAddress addr1(IPv4Address(src1.s_addr));
    ManetAddress addr2(IPv4Address(src2.s_addr));
    struct in_addr myAddr = my_addr();
