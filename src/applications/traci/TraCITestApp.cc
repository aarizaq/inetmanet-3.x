...
void TraCITestApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        if (debug)
            std::cout << "TraCITestApp initialized with testNumber=" << testNumber << std::endl;
    }
}
...
void TraCITestApp::handlePositionUpdate()
{
    ...
    if (testNumber == testCounter++)
    {
        if (t == 1)
        {
            assertClose("(commandDistanceRequest, air)", 859.4556417,
                    traci->commandDistanceRequest(Coord(25, 7030), Coord(883, 6980), false));
            assertClose("(commandDistanceRequest, driving)", 845.93,
                    traci->commandDistanceRequest(Coord(25, 7030), Coord(883, 6980), true));
        }
    }
    ...
    if (testNumber == testCounter++)
    {
        ...
        if (t == 31)
        {
            std::map<std::string, cModule*>::const_iterator i = traci->getManager()->getManagedHosts().find(
                    "testVehicle0");
