 ...
 */
class INET_API UDPBasicP2P2D : public cSimpleModule, protected INotifiable
{
public:
    enum  modoP2P
    {
...
private:
  ...
  protected:
    ...
    cMessage *myTimer;
    cMessage *retryTimer;
    cMessage *informTimeOut;
    FuzzYControl *fuzzy;
    uint64_t mySeqNumber;
  ...
  protected:
    ...
    virtual uint64_t chooseDestAddr(uint32_t segmentId, PathAddress&);
    virtual void getList(ListAddress &);
    virtual void getList(ListAddress &address,uint32_t segmentId);
    virtual void getList(ListAddress &address, std::vector<SegmentList> &);
    ...
    virtual std::vector<UDPBasicP2P2D::InfoData> selectBestList(const ListAddress &address, const std::vector<SegmentList> &List, PathAddress &routeSelected);
    virtual std::vector<UDPBasicP2P2D::InfoData>  chooseDestAddrList();
