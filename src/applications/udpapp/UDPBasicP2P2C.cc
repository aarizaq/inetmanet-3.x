...
simsignal_t UDPBasicP2P2C::queueLengthInstSignal = registerSignal("queueLengthInst");
    ...
    else if (stage == 6)
    {
        ...
        if (par("appRouting").boolValue())
        {
            ...
            for (unsigned int i = 0; i < routing->getNumRoutes(); i++)
            {
                ...
                if (pathNode.empty())
                    continue;
                GlobalWirelessLinkInspector::setRoute(this, myAddress, ManetAddress(pathNode.back()),
                        ManetAddress(pathNode[0]), false);
...
void UDPBasicP2P2C::receiveChangeNotification(int category, const cObject *details)
{
    Enter_Method_Silent();
}
...
long unsigned UDPBasicP2P2C::getQueueSize()
{
...
ManetAddress UDPBasicP2P2C::DelayMessage::getNodeId() const
{
    return nodeId_var;
}

void UDPBasicP2P2C::DelayMessage::setNodeId(ManetAddress nodeId)
{
    this->nodeId_var = nodeId;
}

ManetAddress UDPBasicP2P2C::DelayMessage::getDestination() const
{
    return destination_var;
}

void UDPBasicP2P2C::DelayMessage::setDestination(ManetAddress destination)
{
