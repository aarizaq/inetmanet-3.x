...
void DNSBase::handleMessage(cMessage *msg)
{
    ...
    short respType = 0;
    //CASE 1: HIT -> ip
    if (!dnsMsg->addrData().isUnspecified()) respType = 1;
    ...
    else if (((string)dnsMsg->data()).length() > 3)
    {
      ...
      prefix[4]='\0';
      //CASE 2: name->HIT & ip
      if (strcmp(prefix, "hip-")==0) respType = 2;
      //CASE 3: name->ip
      else respType = 3;
    }
    //CREATING response
	...
	*/
    sendToUDP(respMsg, serverPort, controlInfo->getSrcAddr(), controlInfo->getSrcPort());
