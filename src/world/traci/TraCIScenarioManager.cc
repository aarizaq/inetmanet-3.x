...
{
    if (!socketPtr)
        error("Connection to TraCI server lost");
        ...
        while (bytesRead < sizeof(uint32_t))
        {
            ...
            else
            {
                ...
                if (sock_errno() == EAGAIN)
                    continue;
                error("Connection to TraCI server lost. Check your server's log. Error message: %d: %s", sock_errno(),
                        strerror(sock_errno()));
            }
        }
...
TraCIScenarioManager::TraCIBuffer TraCIScenarioManager::queryTraCI(uint8_t commandId, const TraCIBuffer& buf)
{
    ...
    if (result == RTYPE_NOTIMPLEMENTED)
        error("TraCI server reported command 0x%2x not implemented (\"%s\"). Might need newer version.", commandId,
                description.c_str());
    if (result == RTYPE_ERR)
        error("TraCI server reported error executing command 0x%2x (\"%s\").", commandId, description.c_str());
...
void TraCIScenarioManager::connect()
{
    ...
    if (MYSOCKET < 0)
        error("Could not create socket to connect to TraCI server");
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf)
{
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            error("TraCI server reported error subscribing to variable 0x%2x (\"%s\").", variable1_resp,
                    description.c_str());
        }
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j)
    {
        ...
        if (isokay != RTYPE_OK)
        {
            ...
            if (isSubscribed)
            {
                ...
                error("TraCI server reported error subscribing to vehicle variable 0x%2x (\"%s\").", variable1_resp,
                        errormsg.c_str());
            }
        }
