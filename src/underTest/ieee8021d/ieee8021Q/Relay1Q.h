 ...
 */
class Relay1Q : public cSimpleModule, public ILifecycle
{
    protected:
        Cache1Q *cache; /// SwTable module pointer. Updated every time the admcarelay module is accessed.
        RSTP1Q * rstpModule; /// RSTP module pointer. Updated every time the admcarelay module is accessed.
    ...
    public:
        virtual bool handleOperationStage(LifecycleOperation*, int, IDoneCallback*);
    protected:
        virtual void start();
        virtual void stop();
        bool virtual isNodeUp();
};
