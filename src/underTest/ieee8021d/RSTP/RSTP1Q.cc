...
void RSTP1Q::initialize(int stage)
{
	...
	if(stage==1) // "auto" MAC addresses assignment takes place in stage 0.
	{
		...
		//RSTP1Q module initialization. Puertos will save per port RSTP1Q info.
...
{//Saving statistics
	if(testing==true)
	{
		FILE * pFile;
		char buff[50];
		sprintf(buff,"results/RSTP1Q-%s.info",getParentModule()->getName());
		pFile = fopen (buff,"w");
		  if (pFile!=NULL)
		  {
			int r=getRootIndex();
...
void RSTP1Q::handleMessage(cMessage *msg)
{//It can receive BPDU or self messages. Self messages are hello time, time to switch to designated, or status upgrade time.
	...
	{
		ev<<"BPDU received at RSTP1Q module."<<endl;
...
void RSTP1Q::handleMigrate(cMessage * msg)
{
	for(unsigned int i=0;i<Puertos.size();i++)
	{
...
void RSTP1Q::handleUpgrade(cMessage * msg)
{
	for(unsigned int i=0;i<Puertos.size();i++)
	{
...
void RSTP1Q::handleHelloTime(cMessage * msg)
{
	if(verbose==true)
	{
		ev<<"Hello time."<<endl;
...
void RSTP1Q::checkTC(BPDUieee8021D * frame, int arrival)
{
...
void RSTP1Q::handleBK(BPDUieee8021D * frame, int arrival)
{
...
void RSTP1Q::handleIncomingFrame(Delivery *frame2)
	...
	if((r>=0)&&(r<Puertos.size()))
	{
		if(Puertos[r].PortRole!=DISABLED)
		{
			if(simulation.getSimTime()<Puertos[r].TCWhile)
			{
				...
				RSTPQ1Vector a = getRootRstpVector();
...
void RSTP1Q::initPorts()
{ //Initialize RSTP1Q dynamic information.
	for(unsigned int j=0;j<Puertos.size();j++)
	{
...
PortRoleT RSTP1Q::getPortRole(int index)
{
	Enter_Method("Get rol");
	return Puertos[index].PortRole;
}
PortStateT RSTP1Q::getPortState(int index)
{
	Enter_Method("Get State");
	return Puertos[index].PortState;
}

void RSTPQ1Vector::init(int priority, MACAddress address)
...
RSTPQ1Vector RSTP1Q::getRootRstpVector()
...
int RSTP1Q::contestRstpVector(RSTPQ1Vector vect2,int v2Port)
...
int RSTP1Q::contestRstpVector(BPDUieee8021D* msg,int arrival)
...
int RSTPQ1Vector::compareRstpVector(BPDUieee8021D * msg,int PortCost)
...
int RSTPQ1Vector::compareRstpVector(RSTPQ1Vector vect2)
{//Compares msg with vect. >0 if msg better than vect. =0 if they are similar.
	// -4=Worse Port  -3=Worse Src -2=Worse RPC -1=Worse Root  0= Similar  1=Better Root. 2= Better RPC  3= Better Src   4= Better Port
	int result=0;
...
void RSTP1Q::scheduleUpTimeEvent(cXMLElement * event)
...
void RSTP1Q::handleUpTimeEvent(cMessage * msg)
