...
cMessage * Ieee80211MgmtSTAExtended::enqueue(cMessage *msg)
{
...
cPacket *Ieee80211MgmtSTAExtended::decapsulate(Ieee80211DataFrame *frame)
{
...
void Ieee80211MgmtSTAExtended::clearAPList()
{
    for (AccessPointList::iterator it=apList.begin(); it!=apList.end(); ++it)
        if (it->authTimeoutMsg)
            cancelAndDelete(it->authTimeoutMsg);
    apList.clear();
}
