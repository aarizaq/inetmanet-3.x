...
//If this is an incoming MVRPDU. Not forwarding or learning are ignored.
    if ((rstpModule->getPortState(frame->getArrivalGate()->getIndex()) == FORWARDING)
            || (rstpModule->getPortState(frame->getArrivalGate()->getIndex()) == LEARNING))
    {
        Delivery * frame2 = new Delivery();
        frame2->setArrivalPort(frame->getArrivalGate()->getIndex());
        frame2->encapsulate(frame);
...
bool Relay1Q::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
