...
int WeightedFairQueue::getLength() const
{
    int len = 0;
    for (int i = 0; i < numQueues; i++)
        if (!queueArray[i].empty())
            len += queueArray[i].getLength();
    return len;
}

int WeightedFairQueue::getByteLength() const
{
    int len = 0;
    for (int i = 0; i < numQueues; i++)
    {
        if (!queueArray[i].empty())
        {
            for (cQueue::Iterator it(queueArray[i]); !it.end(); it++)
            {
                cPacket *pk = dynamic_cast<cPacket *>(it());
                if (pk)
                    len += pk->getByteLength();
            }
        }
    }
    return len;
